...
const portfolioItems = [
  {
    id: 1, title: "Neon Drift Hoodie Collection", category: "Clothing",
    tags: ["DTF Print", "Streetwear"],
    gradient: "from-purple-700 via-violet-800 to-indigo-900",
    accentColor: "#a855f7",
    size: "large",
    shape: "circles", // decorative pattern type
  },
  {
    id: 2, title: "AuraFuel Brand Identity", category: "Branding",
  ...
  {
    id: 3, title: "Summit Gear Merch Pack", category: "Merchandise",
    tags: ["Caps", "Tote Bags", "Stickers"],
  ...
  {
    id: 4, title: "Luminos Festival Posters", category: "Illustration",
    tags: ["Poster Art", "Event Branding"],
  ...
  {
    id: 5, title: "Vertex Sports Collection", category: "Clothing",
    tags: ["Jerseys", "Sportswear"],
  ...
  {
    id: 6, title: "Bloom Coffee Brand System", category: "Branding",
  ...
  {
    id: 7, title: "TechNova Swag Kit", category: "Merchandise",
    tags: ["Mugs", "Notebooks", "T-Shirts"],
  ...
  {
    id: 8, title: "Celestial Illustration Series", category: "Illustration",
    tags: ["Digital Art", "Print Series"],
    gradient: "from-violet-700 via-purple-800 to-pink-800",
    accentColor: "#8b5cf6",
    size: "small",
    shape: "hexagon",
  },
...
/** SVG decorative pattern per card */
function CardPattern({ shape, color }: { shape: string; color: string }) {
  const c = color + "40"; // 25% opacity hex

  if (shape === "circles") return (
    <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
      <circle cx="80%" cy="20%" r="100" fill="none" stroke={c} strokeWidth="32" />
      <circle cx="20%" cy="80%" r="60"  fill="none" stroke={c} strokeWidth="24" />
      <circle cx="60%" cy="60%" r="40"  fill="none" stroke={c} strokeWidth="16" />
    </svg>
  );
  ...
  if (shape === "grid") return (
    <svg className="absolute inset-0 w-full h-full opacity-30" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <pattern id={`g-${color}`} x="0" y="0" width="32" height="32" patternUnits="userSpaceOnUse">
          <path d="M 32 0 L 0 0 0 32" fill="none" stroke={c} strokeWidth="1"/>
        </pattern>
      </defs>
  ...
  if (shape === "diagonal") return (
    <svg className="absolute inset-0 w-full h-full opacity-25" xmlns="http://www.w3.org/2000/svg">
      {[...Array(10)].map((_, i) => (
        <line key={i} x1={i * 48 - 100} y1="0" x2={i * 48 + 100} y2="400" stroke={c} strokeWidth="1.5" />
      ))}
    </svg>
  );
  ...
  if (shape === "hexagon") return (
    <svg className="absolute inset-0 w-full h-full opacity-20" xmlns="http://www.w3.org/2000/svg">
      <polygon points="85,0 170,42 170,126 85,168 0,126 0,42" fill="none" stroke={c} strokeWidth="1.5" transform="translate(60,30)" />
      <polygon points="60,0 120,30 120,90 60,120 0,90 0,30" fill="none" stroke={c} strokeWidth="1" transform="translate(170,100)" />
    </svg>
  );
  ...
  return (
    <div className="min-h-screen pt-32 pb-24 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Filter tabs */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-wrap justify-center gap-2.5 mb-12">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`relative px-5 py-2 rounded-full text-sm font-medium transition-all duration-250 overflow-hidden ${
                active === cat
            ...
            >
              {active === cat && (
                <motion.span layoutId="filter-pill" className="absolute inset-0 bg-gradient-to-r from-purple-600 to-blue-600 rounded-full" style={{ zIndex: -1 }} transition={{ type: "spring", stiffness: 380, damping: 30 }} />
              )}
              {cat}
            </button>
          ))}
          ...
          {filtered.map((item, i) => (
            <motion.div
              ...
              className={`group relative overflow-hidden rounded-2xl cursor-pointer ${
                item.size === "large" ? "sm:col-span-2 lg:col-span-2 min-h-[300px]" : "min-h-[250px]"
              }`}
            >
              {/* Base gradient */}
              ...
              <div className="relative z-10 p-7 h-full flex flex-col justify-end">
                ...
                <h3 className="font-display text-lg font-bold text-white mb-1 leading-snug">{item.title}</h3>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mt-20">
          <p className="text-[var(--text-secondary)] mb-5 text-lg">Love what you see? Let&apos;s build something together.</p>
